...
public class ClientCommandUtil {
    ...
    public static void init() {
        packagers.put(ClientCommandPackager4App.name,
                new ClientCommandPackager4App());
        packagers.put(ClientCommandPackager4Query.name,
                new ClientCommandPackager4Query());
        packagers.put(ClientCommandPackager4Report.name,
                new ClientCommandPackager4Report());
        packagers.put(ClientCommandPackager4FileTransfer.name,
                new ClientCommandPackager4FileTransfer());
        initialed = true;
    }
    ...
    public static void parseCommand(String paraName, String value, ICommand c) {
        if (!initialed)
            init();
        ((IClientCommandPackager) packagers.get(c.getType())).parseCommand(
                paraName, value, c);
    }

    // public static void packCommand(Packet p,Command c){
    //
    // Element e=p.getElement().addElement(Constants.XMPP_COMMAND,
    // Constants.XMPP_NAMESPACE);
    ...
    public static void packCommand(String type,
            org.jivesoftware.smack.packet.IQ p, ICommand c) {
        if (!initialed)
            init();
    ...
    // public static Command parseCommand(org.jivesoftware.smack.packet.Packet
    // p){
    // System.out.println(p.getExtension(Constants.XMPP_COMMAND,
    // Constants.XMPP_NAMESPACE).getClass().getName());
    // DefaultPacketExtension
    // ex=(DefaultPacketExtension)p.getExtension(Constants.XMPP_COMMAND,
    // Constants.XMPP_NAMESPACE);
    ...
    //
    // Application app=new
    // Application(ex.getValue("appname"),ex.getValue("version"),ex.getValue("url"));
    // command.setApp(app);
    //
    // break;
    // }
