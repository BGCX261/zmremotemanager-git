...
public class LogManager implements NetworkStatusMonitor.NetworkStatusReport{
    ...
    static private SimpleDateFormat mSimpleDateFormat = new SimpleDateFormat("yyyy-MM-dd HH:mmZ");
    ...
    private void transferLeftOverLogs(){
        try {
            ...
            if(mLocalLogFile.length() != 0){
                Log.e(TAG, "transfer leftover logs to server, size :" + leftSize);
                BufferedReader reader = new BufferedReader(new FileReader(mLocalLogFile));
                ...
                reader.close();
                if(bInterrupted){
                    Log.e(TAG, "transfer leftover logs to server is interrupted" );
                }else{
                    mLocalLogFile.delete();//delete the old file as leftover logs have been sent to server
                    mLocalLogFile = mContext.getDir("local.log", Context.MODE_PRIVATE);
                    mLocalLogFileWrite = new BufferedWriter(new FileWriter(mLocalLogFile));
                }
            }
        } catch (Exception e) {
            // TODO: handle exception
